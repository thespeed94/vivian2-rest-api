$(document).ready(function(){
    ...
    let tabla = $('#tCategoriesProduct').DataTable({
        columnDefs: [
            {  targets: 3,
    ...
    $("#agregarCategoriesProduct").click(function (){
        $('#btnGuardarCategoriesProduct').val("agregar");
    });
    ...
    $('#tCategoriesProduct tbody').on('click', '.editar', function () {
   
        $("label.error").remove();
        
        var id = $(this).attr("id").match(/\d+/)[0];
        var data = $('#tCategoriesProduct').DataTable().row( id ).data();
        ...
        openModal();
        $('#btnGuardarCategoriesProduct').val("editar");
    });

    // METODO PARA MOSTRAR UNA ALERTA PARA POSTERIORMENTE ELIMINAR UN REGISTRO
    $('#tCategoriesProduct tbody').on('click', '.eliminar', function () {
        var id = $(this).attr("id").match(/\d+/)[0];
        var data = $('#tCategoriesProduct').DataTable().row( id ).data();
        ...
        $.ajax({
            ...
            data: {
                ...
                if (data.estado === 1){
                    ...
                    parent.location.href="/categoria";
                }else{
    ...
    $("#formCategoriesProduct").validate({
        rules:{
            descripcionCategoria: { required: true, maxlength:45},
            estado: { required: true}
            
            },
        messages:{
            descripcionCategoria: { required:"El campo es requerido", maxlength:'Máximo 45 caracteres'},
    ...
    $('#btnGuardarCategoriesProduct').on('click',function(){
        if($('#formCategoriesProduct').valid() == false){
            return false;
        }else{
            // SI EL VALUE ES agregar INGRESA UN NUEVO USUARIO (POST)
            if ($('#btnGuardarCategoriesProduct').val() === "agregar"){
                $.ajax({
                    type: 'POST',
                    url: '/categoria',
                    data: {
                        "descripcionCategoria": $("#txtDescripcion").val(),
            ...
            } else if ($('#btnGuardarCategoriesProduct').val() === "editar"){
